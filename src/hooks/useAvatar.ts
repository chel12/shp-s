...
const useAvatar = ({ userId, gender = 'male' }: UseAvatarProps) => {
	...
	const loadAvatar = useCallback(async () => {
		...
		try {
			...
			if (response.ok) {
				...
				if (blob.size > 0) {
					//если blob формат есть то делаем временную ссылку
	...
	//компонент будет монтироваться до того как данные прийдут с сервера
	//поэтому загрузим когда компонент смонтируется
	useEffect(() => {
		loadAvatar();
	}, [loadAvatar]);

	//очистка при размонтирование
	useEffect(() => {
		return () => {
