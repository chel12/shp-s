...
const DeliveryTime = ({
	...
	//получет генерацих 3 дней из мапина ниже
	const [availableDates, setAvailableDates] = useState<
		{ value: string; label: string }[]
	>([]);
	//для вывода тул типов
	const [tooltipSlot, setTooltipSlot] = useState<string | null>(null);
	//получаем график досавки с апишки и кидаем сюда
	...
	useEffect(() => {
		//получение дат на 3 дня вперёд и форматирование их
	...
	const getAllTimeSlots = () => {
		...
		const slots = Object.keys(daySchedule)
			...
			.map((slot) => {
				...
				return {
					value: slot, //исходное
					mobileLabel: formatted.mobileLabel, //мобилкам
					desktopLabel: formatted.desktopLabel, //деску
					free: isAvailable, //свободно или нет
					passed: isPassed, //время прошло
				};
			});
		return slots;
	};
	...
	//вызываем функцию которая вверху чтобы затём отрендерить
	...
	return (
		<div>
			...
			<div
				className="relative flex flex-col gap-y-4 md:flex-row
			 md:flex-nowrap md:gap-x-8 xl:gap-x-10">
				<div>
					...
					<select
						...
						className={`${formStyles.input} ${additionalStyles} ${selectStyles} [&&]:md:w-[155px]
						 [&&]:text-base`}>
