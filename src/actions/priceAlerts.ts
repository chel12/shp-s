...
export async function createPriceAlert(
	...
	try {
		...
		//ещё раз проверка на емаил корректный
		...
		if (existingAlert) {
			return {
				errors: {
					email: 'Вы уже подписаны на уведомления для этого товара',
				},
			};
		}
		//генерация токена чтобы отслеживать конкретный продукт
